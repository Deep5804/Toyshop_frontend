...
// // Add at the beginning of your handleQuantityChange function
// const handleQuantityChange = (uniqueId, quantity) => {
//   console.log("Changing quantity for unique ID:", uniqueId, "to", quantity);
  
//   if (quantity >= 1 && quantity <= 20) {
//     // Find the item using the unique ID
//     const item = cartItems.find(item => 
//       item.uniqueCartId === uniqueId || 
//       `cart-item-${cartItems.indexOf(item)}-${item.productID || item.productId || item._id}` === uniqueId
//     );
    
//     if (item) {
//       // Use the actual productID for the Redux action
//       const productID = item.productID || item.productId || item._id;
//       dispatch(updateQuantity({ 
//         productID: productID, 
//         uniqueId: uniqueId, // Pass the unique ID too
//         quantity: quantity 
//       }));
//     }
//   }
...
//         // Check if we already have items in the cart (added from home page)
//         if (cartItems && cartItems.length > 0) {
//           console.log("Using existing cart items:", cartItems);
//           return; // Skip API fetch if we already have items
//         }
...
//           // In your fetchCart function, when mapping items:
// // In your fetchCart function, when mapping items:
...
//       return {
//         uniqueCartId: `cart-item-${index}-${productId}`, // Add unique identifier
//         productID: productId,
...
//       // Determine the correct ID to use for this item
//       const itemId = item.productID || item.productId || item._id;
//       const uniqueId = item.uniqueCartId || `cart-item-${index}-${item.productID || item.productId || item._id}`;
...
//                   await fetch(`https://toyshop-sooty.vercel.app/api/cart/${userId}`, {
//                     method: "PUT",
//                     headers: {
                                ...
                                onClick={async (e) => {
                                  ...
                                  try {
                                    ...
                                    if (updatedCart.cart && updatedCart.cart.items) {
                                      const updatedItems = updatedCart.cart.items.map((item, index) => ({
                                        uniqueCartId: `cart-item-${index}-${item.productId._id || item.productId}`,
